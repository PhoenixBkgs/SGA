...
void ImageObject::SpritesRender(HDC hdc, RECT SpritesBox, BYTE alpha)
{
    ...
    GdiTransparentBlt(hdc, SpritesBox.left, SpritesBox.top, 
                           boxSize.x, boxSize.y, 
                           m_pImageInfo->hMemDC, 
                           m_spritesWidth * m_currFrameX, m_spritesHeight * m_currFrameY,
                           m_spritesWidth, m_spritesHeight
                        , m_transColor);
}

void ImageObject::SpritesRender(HDC hdc, RECT SpritesBox, int SrcPosX, int SrcPosY, BYTE alpha)
{
#ifdef _DEBUG
    ...
    GdiTransparentBlt(hdc
                    ...
                    , boxSize.x, boxSize.y
                    , m_pImageInfo->hMemDC
                    , m_spritesWidth * SrcPosX, m_spritesHeight * SrcPosY
                    , m_spritesWidth, m_spritesHeight
                    , m_transColor);
}
...
void ImageObject::DrawOutline(RECT Box)
{
    if (m_drawHelper != NULL)
    {
        m_drawHelper->DrawBoxLine2D(Box, 5, _RGBA{ 0, 255, 0, 0 });
    }
}